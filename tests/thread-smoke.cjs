...
function fixture(id, state, page) {
  ...
  const reply = parent ? `<a href="https://twitter.com/demo/status/${parent}">返信先: @demoさん</a>` : '';
...
async function capture(url, mode='embed') {
  const job = {cancelled:false,window:null};
  let inspect;
  const result=await capturePost(parsePostUrl(url),validateOptions({conversation:'thread',mode}),job,(message,percent)=>{
